...
def get_issue_message_chatbot_response(retrieval_chain, conversation_chain, issue_information):
    try:
        ...
        troubleshoot_prompt = """System Prompt: \nYou are a helpful IBM MQ AI assistant. Give an overview of the problem in my system and how to fix it (include IBM Documentation hyperlinks where possible). 
        \nSystem Issue Message:\n""" + issue_information

        result = retrieval_chain({"question": troubleshoot_prompt})['answer']
...
def get_general_chatbot_response(retrieval_chain, conversation_chain, user_query):
    try:
        ...
        troubleshoot_prompt = """System Prompt: \nYou are a helpful IBM MQ AI assistant. If the user's query relates to 'IBM MQ', answer informatively. For queries not about IBM MQ or general pleasantries, reply that the question does not relate to IBM MQ and so you cannot answer it. 
        \nUser Prompt:\n""" + user_query
